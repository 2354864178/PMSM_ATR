...
// 涡轮模型：
// - 状态主变量：p_plenum（前腔压力）、p_downstream（后腔压力）
// - 代数量：m_dot_in、m_dot_turb、m_dot_out、T_turb
class TurbineModel {
public:
    struct Command {
    ...
    struct GasEval {
        ...
        double dp_downstream = 0.0;
        double m_dot_in = 0.0;
        double m_dot_turb = 0.0;
        double m_dot_out = 0.0;
        double T_turb = 0.0;
    };
    ...
    double V_downstream = 3e-3;     // 后腔体积 m^3
    double C_in = 1.0;              // 入口流量系数
    double Cd_nozzle = 0.85;        // 前腔到后腔喷嘴流量系数
    double A_nozzle = 1.2e-4;       // 前腔到后腔喷嘴等效面积 m^2
    double Cd_outlet = 0.8;         // 后腔出口阀/喷嘴流量系数
    double A_outlet = 8e-5;         // 后腔出口阀/喷嘴等效面积 m^2
    double p_floor = 20000.0;       // 压力下限保护 Pa

    // 容积模型状态/中间量
    double p_plenum = 150000.0;     // 腔体压力状态 Pa
    double p_downstream = 100000.0; // 后腔压力状态 Pa
    double m_dot_in = 0.0;          // 进腔质量流量 kg/s
    double m_dot_turb = 0.0;        // 通过涡轮质量流量 kg/s
    double m_dot_out = 0.0;         // 后腔外流质量流量 kg/s

    double T_turb = 0.0;            // 轴扭矩 N·m

    // 纯计算：输入候选状态与边界命令，输出导数与扭矩。
    GasEval evaluate_gas(double p_plenum_in,
                         double p_downstream_in,
                         double omega_shaft,
                         const Command& cmd) const;
    // 回写计算结果，供系统其他层读取。
    void apply_gas_state(double omega_shaft, double p_plenum_in, double p_downstream_in, const GasEval& eval);
    // 参数下发。
